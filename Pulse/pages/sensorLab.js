...
Page({
  build() {
    this.mode = 'home'; this.lastUiUpdate = 0; this.calibrationCandidates = [];
    this.calibration = null; this.bottomGravity = null; this.setReps = []; this.debugSamples = []; this.countdownTimer = null;
  ...
  handleAction() {
    ...
    if (this.mode === 'bicep-ready' || this.mode === 'calibration-complete') return this.startCalibration();
    if (this.mode === 'set-ready') return this.startSet();
  ...
  enterHome() {
    this.clearCountdown(); this.mode = 'home'; this.sensorManager.stopRecording(); this.releaseScreenAwake();
  ...
  enterBicepCurl() {
    this.mode = this.calibration ? 'calibration-complete' : 'bicep-ready'; this.titleText.setProperty(prop.MORE, { text: 'BICEP CURL' });
    this.primaryText.setProperty(prop.MORE, { text: this.calibration ? 'Calibration complete' : 'Calibration: 3-5 controlled reps' });
    this.secondaryText.setProperty(prop.MORE, { text: this.calibration ? 'Start a set when ready' : 'Press CALIBRATE and curl normally' });
    this.setButtonText(this.actionButton, 'CALIBRATE'); this.setButtonText(this.stopButton, 'START SET');
    this.setStatus(this.calibration ? `Baseline ${this.calibration.baselineVelocity.toFixed(2)}x proxy` : 'Calibration required before set tracking.');
  },
  startSensorLabRecording() { this.sensorManager.startRecording(); this.setStatus('Recording raw Sensor Lab data.'); },
  startCalibration() {
    this.mode = 'calibration-countdown'; this.keepScreenAwake(true); this.titleText.setProperty(prop.MORE, { text: 'GET READY' });
    this.setButtonText(this.actionButton, 'WAIT'); this.setButtonText(this.stopButton, 'CANCEL'); this.startCountdown('calibration');
  },
  ...
  startSet() {
    if (!this.calibration) { this.setStatus('Calibration is required before starting a set.'); return; }
  ...
  beginSetTracking() {
    this.mode = 'set'; this.setReps = []; this.debugSamples = []; this.processor.reset(); this.processor.setBottomReference(this.calibration.bottomGravity); this.repDetector.reset(this.calibration); this.sensorManager.startRecording({ maxSamples: CONSTANTS.MAX_RAW_SAMPLES, storeSamples: false });
    this.titleText.setProperty(prop.MORE, { text: 'BICEP CURL SET' }); this.primaryText.setProperty(prop.MORE, { text: 'Reps: 0 | Valid: 0' });
    this.secondaryText.setProperty(prop.MORE, { text: 'Relative concentric velocity only' }); this.setButtonText(this.actionButton, 'TRACKING');
    this.setButtonText(this.stopButton, 'STOP / SAVE'); this.playStartSignal(); this.setStatus('Set started. Move through full controlled curls.');
  },
  ...
  processWorkoutSample(rawSample) {
    if (this.mode !== 'calibration-bottom' && this.mode !== 'calibrating' && this.mode !== 'set') return;
    this.keepScreenAwake(false);
    const processed = this.processor.process(rawSample); if (!processed) return;
    if (this.mode === 'calibration-bottom') {
      ...
      if (rawSample.timestamp - this.bottomReferenceStartedAt >= CONSTANTS.BOTTOM_REFERENCE_DURATION_MS) {
        ...
        this.processor.setBottomReference(this.bottomGravity); this.repDetector.reset(null); this.mode = 'calibrating';
        this.primaryText.setProperty(prop.MORE, { text: 'Perform 3 controlled curls' }); this.secondaryText.setProperty(prop.MORE, { text: 'Reach top, then return down' }); this.setStatus('Bottom position captured. 0 valid calibration reps.');
      }
      return;
    }
  ...
  acceptSetRep(rep) {
    rep.repNumber = this.setReps.length + 1; rep.baselineVelocity = this.calibration.baselineVelocity;
    Object.assign(rep, FatigueEngine.evaluate(rep, this.calibration.baselineVelocity)); delete rep.samples; delete rep.phase; this.setReps.push(rep);
    const valid = this.setReps.filter((entry) => entry.valid).length; this.primaryText.setProperty(prop.MORE, { text: `Reps: ${this.setReps.length} | Valid: ${valid}` });
    if (rep.valid) this.playRepSignal();
    this.secondaryText.setProperty(prop.MORE, { text: `${rep.normalizedConcentricVelocity.toFixed(2)}x | loss ${rep.velocityLossPercent.toFixed(1)}%` }); this.setStatus(`Rep ${rep.repNumber}: ${rep.valid ? 'valid' : 'rejected'}, confidence ${rep.confidence}.`);
  },
  stopAndSave() {
    ...
    if (this.mode === 'calibration-countdown' || this.mode === 'set-countdown' || this.mode === 'calibration-bottom' || this.mode === 'calibrating') { this.clearCountdown(); this.sensorManager.stopRecording(); this.releaseScreenAwake(); this.enterBicepCurl(); return; }
    if (this.mode === 'set') {
      this.sensorManager.stopRecording(); const metrics = FatigueEngine.summarize(this.setReps, this.calibration.baselineVelocity);
      const workout = { schemaVersion: CONSTANTS.STORAGE_SCHEMA_VERSION, exercise: 'bicepCurl', createdAt: Date.now(), calibration: this.calibration, reps: this.setReps, metrics, debug: CONSTANTS.DEBUG_ENABLED ? this.debugSamples : undefined };
      const saved = WorkoutStorage.saveWorkout(workout); this.releaseScreenAwake(); this.mode = 'set-complete'; this.titleText.setProperty(prop.MORE, { text: 'SET RESULT' });
      this.primaryText.setProperty(prop.MORE, { text: `${metrics.validReps}/${metrics.totalDetectedReps} valid | ${metrics.fatigueLevel}` }); this.secondaryText.setProperty(prop.MORE, { text: `Average loss ${metrics.averageVelocityLoss.toFixed(1)}%` });
      this.setButtonText(this.actionButton, 'BICEP CURL'); this.setButtonText(this.stopButton, 'HOME'); this.setStatus(saved ? `Saved. Kinematic fatigue estimate: ${metrics.fatigueLevel} (${metrics.fatigueScore}).` : 'Set ended but storage failed.'); return;
    }
    this.enterHome();
  },
  ...
  onDestroy() { this.clearCountdown(); this.releaseScreenAwake(); if (this.sensorManager) this.sensorManager.stop(); }
});
